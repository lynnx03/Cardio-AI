...
@app.get("/")
def root():
    # เสิร์ฟหน้าเว็บ — Docker ใช้ static/index.html, รัน local ใช้ real.html ตรงๆ
    for path in ("static/index.html", "real.html"):
        if os.path.exists(path):
            return FileResponse(path)
    raise HTTPException(status_code=404, detail="ไม่พบไฟล์หน้าเว็บ (static/index.html หรือ real.html)")
